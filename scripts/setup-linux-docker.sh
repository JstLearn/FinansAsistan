#!/bin/bash
# ════════════════════════════════════════════════════════════
# FinansAsistan - Linux Docker Compose Setup Script
...
echo ""
echo "═══════════════════════════════════════════════════════════"
echo "  FinansAsistan - Linux Docker Compose Setup"
echo "═══════════════════════════════════════════════════════════"
echo ""
...
if [ -z "$AWS_REGION" ]; then
    log_error "AWS_REGION environment variable must be set"
    exit 1
fi
if [ -z "$S3_BUCKET" ]; then
    log_error "S3_BUCKET environment variable must be set"
    exit 1
fi
...
# Load .env file from QUICK_START directory (if not already loaded)
...
register_leadership() {
    ...
    if ! command -v jq &> /dev/null; then
        ...
        if command -v apt-get &> /dev/null; then
            ...
            log_warn "Cannot install jq automatically. Please install jq manually."
            return 0
        fi
    fi
            ...
            if [ -n "$CURRENT_LEADER_JSON" ]; then
                ...
                if [ -n "$CURRENT_LEADER_ID" ] && [ "$CURRENT_LEADER_ID" != "$MACHINE_ID" ]; then
                    ...
                    if [ -n "$CURRENT_LAST_HEARTBEAT" ]; then
                        ...
                        CURRENT_HEARTBEAT_EPOCH=$(date -u -d "$CURRENT_LAST_HEARTBEAT" +%s 2>/dev/null || echo "0")
                        NOW_EPOCH=$(date -u +%s)
