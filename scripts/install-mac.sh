#!/bin/bash
# ════════════════════════════════════════════════════════════
# FinansAsistan - One-Line macOS Installer
...
if ! command_exists git; then
    ...
    if command_exists brew; then
        ...
        echo "[ERROR] Cannot install Git automatically. Please install Git manually or install Homebrew first."
...
install_argocd_cli() {
    ...
    # Try Homebrew first
    if command_exists brew; then
        echo "[INFO] Attempting to install ArgoCD CLI via Homebrew..."
        if brew install argocd; then
            if argocd version --client >/dev/null 2>&1; then
                echo "[SUCCESS] ArgoCD CLI installed via Homebrew"
                return 0
            fi
        fi
    fi
...
SETUP_SCRIPT="/tmp/setup-mac-docker.sh"

# Get file content from GitHub API (base64 encoded)
API_RESPONSE=$(curl -s -H "Authorization: token $TOKEN" \
     -H "Accept: application/vnd.github.v3+json" \
     "https://api.github.com/repos/JstLearn/FinansAsistan/contents/scripts/setup-mac-docker.sh")
