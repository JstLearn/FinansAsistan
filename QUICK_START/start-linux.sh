#!/bin/bash
# ============================================================
# FinansAsistan - Linux Başlatma (Unified)
# 
...
echo ""
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  FinansAsistan - Ortam ve İşlem Seçimi (Linux)${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
echo ""
...
echo ""
echo -e "${CYAN}═══════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  FinansAsistan - Linux Başlatma${NC}"
    ...
    if [ -z "$S3_BUCKET" ]; then
        ...
        if [ -n "$LEADER_INFO" ]; then
            ...
            if [ -n "$LAST_HEARTBEAT" ]; then
                # Heartbeat kontrolu (son 5 dakika icinde)
                # ISO 8601 format: 2025-12-08T10:30:15Z
                if command -v date &> /dev/null; then
                    # Try GNU date first (Linux)
                    HEARTBEAT_EPOCH=$(date -d "$LAST_HEARTBEAT" +%s 2>/dev/null || echo "0")
                    # If failed, try with UTC conversion
                    if [ "$HEARTBEAT_EPOCH" = "0" ]; then
                        HEARTBEAT_EPOCH=$(date -u -d "$LAST_HEARTBEAT" +%s 2>/dev/null || echo "0")
                    fi
                else
                    HEARTBEAT_EPOCH="0"
                fi
...
if [ -n "$AWS_ACCESS_KEY_ID" ] && [ -n "$AWS_SECRET_ACCESS_KEY" ]; then
    AWS_TOKEN_BASE64=$(echo -n "{\"accessKeyId\":\"$AWS_ACCESS_KEY_ID\",\"secretAccessKey\":\"$AWS_SECRET_ACCESS_KEY\",\"region\":\"$AWS_REGION\"}" | base64 -w 0)
else
    AWS_TOKEN_BASE64=""
fi
...
if [ -n "$AWS_TOKEN_BASE64" ] && [ -n "$S3_BUCKET" ]; then
    curl -s -H "Authorization: token $GITHUB_TOKEN" -H "Accept: application/vnd.github.v3+json" \
      https://api.github.com/repos/JstLearn/FinansAsistan/contents/scripts/install-linux.sh | \
