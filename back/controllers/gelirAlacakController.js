...
const addGelirAlacak = async (req, res) => {
  try {
    ...
    publishEventAsync(TRANSACTION_TOPICS.INCOME_CREATED, {
      id: result.id, kullanici, ekleyen_kullanici, miktar, para_birimi, tahsilat_tarihi,
      taksit, faiz_uygulaniyormu, alindi_mi, talimat_varmi, bagimli_oldugu_gider, aciklama, miktar_belirsiz, tarih: result.tarih
    ...
    res.status(201).json({ 
      ...
      data: { id: result.id, tarih: result.tarih, miktar, para_birimi, tahsilat_tarihi, kullanici, ekleyen_kullanici }
    });
  } catch (err) {
    console.error("SQL Hatası (gelir-alacak):", err);
    res.status(500).json({ success: false, error: err.message || "Gelir-Alacak eklenirken bir hata oluştu" });
  }
};
...
const getAllGelirAlacak = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: err.message || "Gelir-Alacak verileri alınırken bir hata oluştu" });
  }
};
...
const updateGelirAlacak = async (req, res) => {
  try {
    const { id } = req.params;
    let {
      miktar, para_birimi = "TRY", tahsilat_tarihi, taksit = 1,
      faiz_uygulaniyormu = false, alindi_mi = false, talimat_varmi = false,
      bagimli_oldugu_gider = null, aciklama = null, miktar_belirsiz = false
    } = req.body;
    ...
    const kullanici = req.user.username;
    const result = await transaction(async (client) => {
      const checkResult = await client.query('SELECT * FROM gelir_alacak WHERE id = $1 AND kullanici = $2', [id, kullanici]);
      if (checkResult.rows.length === 0) throw new Error('Gelir-Alacak bulunamadı veya yetkiniz yok');
      const updateResult = await client.query(`
        UPDATE gelir_alacak
        SET miktar = $1, para_birimi = $2, tahsilat_tarihi = $3, taksit = $4,
            faiz_uygulaniyormu = $5, alindi_mi = $6, talimat_varmi = $7,
            bagimli_oldugu_gider = $8, aciklama = $9, miktar_belirsiz = $10
    ...
    publishEventAsync(TRANSACTION_TOPICS.INCOME_UPDATED, {
      id: result.id, kullanici, miktar, para_birimi, tahsilat_tarihi, taksit,
      faiz_uygulaniyormu, alindi_mi, talimat_varmi, bagimli_oldugu_gider, aciklama, miktar_belirsiz, tarih: result.tarih
    }, { correlationId: req.correlationId || req.id, ip: req.ip || req.connection.remoteAddress, userAgent: req.get('user-agent') || '', userId: req.user?.id || req.user?.username });

    res.status(200).json({ success: true, message: "Gelir-Alacak başarıyla güncellendi!", data: { id: result.id, tarih: result.tarih, miktar, para_birimi, tahsilat_tarihi, kullanici } });
  } catch (err) {
    console.error("Hata oluştu (update-gelir-alacak):", err);
    res.status(500).json({ success: false, error: err.message || "Gelir-Alacak güncellenirken bir hata oluştu" });
  }
};
...
const deleteGelirAlacak = async (req, res) => {
  try {
    ...
    const deletedRecord = await transaction(async (client) => {
      const checkResult = await client.query('SELECT * FROM gelir_alacak WHERE id = $1 AND kullanici = $2', [id, kullanici]);
      if (checkResult.rows.length === 0) throw new Error('Gelir-Alacak bulunamadı veya yetkiniz yok');
      await client.query('DELETE FROM gelir_alacak WHERE id = $1 AND kullanici = $2', [id, kullanici]);
    ...
    res.status(200).json({ success: true, message: "Gelir-Alacak başarıyla silindi!" });
  } catch (err) {
    console.error("Hata oluştu (delete-gelir-alacak):", err);
    if (err.message === 'Gelir-Alacak bulunamadı veya yetkiniz yok') {
      return res.status(404).json({ success: false, error: err.message });
    }
    res.status(500).json({ success: false, error: err.message || "Gelir-Alacak silinirken bir hata oluştu" });
  }
};

module.exports = { addGelirAlacak, getAllGelirAlacak, updateGelirAlacak, deleteGelirAlacak };
