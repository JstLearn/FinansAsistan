    ...
    try {
        ...
        res.status(201).json({
            ...
            data: {
                id: result.id,
                tarih: result.tarih,
                hatirlatilacak_olay: hatirlatilacak_olay,
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Hatırlatma eklenirken bir hata oluştu'
        });
    }
};
...
const getAllHatirlatma = async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Hatırlatma verileri alınırken bir hata oluştu'
        });
    }
};
    ...
    try {
        ...
        const result = await transaction(async (client) => {
            ...
            const checkResult = await client.query(
                'SELECT * FROM hatirlatmalar WHERE id = $1 AND kullanici = $2',
                [id, kullanici]
            );

            if (checkResult.rows.length === 0) {
                throw new Error('Hatırlatma bulunamadı veya yetkiniz yok');
            }

            // Güncelle
            const updateResult = await client.query(`
                UPDATE hatirlatmalar
                SET hatirlatilacak_olay = $1,
                    olay_zamani = $2
        ...
        res.status(200).json({
            ...
            data: { id: result.id, tarih: result.tarih, hatirlatilacak_olay, olay_zamani, kullanici }
        });
    } catch (error) {
        console.error('SQL Hatası:', error);
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Hatırlatma güncellenirken bir hata oluştu'
        });
    }
};
...
const deleteHatirlatma = async (req, res) => {
    try {
        ...
        await transaction(async (client) => {
            ...
            // Sil
            await client.query(
                'DELETE FROM hatirlatmalar WHERE id = $1 AND kullanici = $2',
                [id, kullanici]
            );
        });
        ...
        res.status(500).json({
            success: false,
            error: error.message || 'Hatırlatma silinirken bir hata oluştu'
        });
    }
};
