...
const addVarlik = async (req, res) => {
  try {
    const {
      ...
      alis_tarihi,
      alis_fiyati,
      alis_para_birimi = null,
      alis_adedi = null,
      saklanildigi_yer = null,
      link = null,
      metrekare = null,
      aciklama = null
    } = req.body;
    ...
    // ✅ Kafka event publish (fire-and-forget, non-blocking)
...
const updateVarlik = async (req, res) => {
  try {
    ...
    const result = await transaction(async (client) => {
      // Önce kayıt var mı ve kullanıcıya ait mi kontrol et
      const checkResult = await client.query(
        'SELECT id FROM varliklar WHERE id = $1 AND kullanici = $2',
        [id, kullanici]
      );
    ...
    // ✅ Kafka event publish (fire-and-forget)
    ...
    res.status(500).json({
      success: false,
      error: err.message || "Varlık güncellenirken bir hata oluştu"
    });
  }
};
...
const deleteVarlik = async (req, res) => {
  try {
    ...
    const deletedRecord = await transaction(async (client) => {
      // Önce kayıt var mı ve kullanıcıya ait mi kontrol et
      const checkResult = await client.query(
        'SELECT id, kategori, varlik FROM varliklar WHERE id = $1 AND kullanici = $2',
        [id, kullanici]
      );
      ...
      // Veriyi sil
      await client.query(
        'DELETE FROM varliklar WHERE id = $1 AND kullanici = $2',
        [id, kullanici]
      );
      
      return checkResult.rows[0]; // Silinen kayıt bilgisi
    });
    ...
    publishEventAsync(TRANSACTION_TOPICS.ASSET_DELETED, {
      id: parseInt(id),
      kullanici,
      kategori: deletedRecord.kategori,
      varlik: deletedRecord.varlik
    }, {
      correlationId: req.correlationId || req.id,
      ip: req.ip || req.connection.remoteAddress,
      userAgent: req.get('user-agent') || '',
    ...
    res.status(500).json({
      success: false,
      error: err.message || "Varlık silinirken bir hata oluştu"
    });
  }
};
